...
class Test_Model_V1_Comment extends TestCase {
	...
	*/
	...
	/**
	 * use test get comments of user  ok
	 * compare with count result > 0
	 * @group get_all_user_comment
	 * @dataProvider user_comments_provider
	 */
	public function test_get_all_user_comments_ok($test_data) {
	
		$author_id = $test_data['author_id'];
		$rs = Comment::get_all_post_comments($author_id);
		$dem = count($rs);
		$this->assertGreaterThan(0, $dem);
	...
	public function test_get_all_user_comments_notok() {
		...
		$author_id = 0;//author id not exist in comment table
